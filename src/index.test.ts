...
describe("BaseStore", () => {
	...
	test("bulk added records are committed atomically", async () => {
		...
		await expect(
			store.bulkAdd<Note>([{ title: "One" }, { title: "Two" }]),
	...
	test("records can be traversed with cursor helpers", async () => {
		...
		await expect(
			collect(
				store.reduce<Note, string>(
					(note) => note.title.length === 3,
					[(note) => note.title],
				),
			),
	...
	test("records can be updated without assuming an id field", async () => {
		...
		await expect(
			store.update<DocumentRecord>("a", { title: "Updated alpha" }),
		).resolves.toBe(true);
		await expect(
			store.update<DocumentRecord>("missing", { title: "Missing" }),
		).resolves.toBe(false);
		await expect(store.updateByPath("b", "meta.status", "ready")).resolves.toBe(
			true,
		);
		await expect(
			store.updateAll<DocumentRecord>({ title: "Every document" }),
		).resolves.toBe(2);
		await expect(store.updateAllByPath("meta.status", "synced")).resolves.toBe(
			2,
		);
